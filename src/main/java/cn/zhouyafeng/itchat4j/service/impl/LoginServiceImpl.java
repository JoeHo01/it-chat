...
public class LoginServiceImpl implements ILoginService {
	...
	private Map<String, GroupInfo> groups = GroupInfo.getInstance();
			...
			for (int i = 0; i < contactList.size(); i++) { // 群好友
				if (contactList.getJSONObject(i).getString("UserName").contains("@@")) { // 群
					...
					for (int j = 0; j < memberList.size(); j++) {
						RecommendInfo recommendInfo = memberList.getObject(j, RecommendInfo.class);
						group.getMembers().put(recommendInfo.getUserName(), recommendInfo);
					}
